...
近期资金水位采用无前视自适应口径：对最近已公布结果的历史新股，逐只用它之前的样本重建当时预测，以“真实冻资 / 未做水位修正的预测冻资”生成残差因子；同一结果日样本不互相引用。系统按样本半衰期取加权中位数，与 `subscription_prediction_recent_market_level_factor` 基准因子收缩混合，并受配置上下限保护。样本不足时只用基准因子；样本齐备后，正式预测和调参回放都使用同一套滚动因子。默认 `coarse-fine` 搜索还会调节是否启用、近期窗口、最少样本、半衰期和自适应权重，候选排序会同时考虑全样本 MAPE、最近 6 只 MAPE 和近期有符号偏差。
