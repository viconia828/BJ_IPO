...
- 双击 [运行.bat](运行.bat)
- 或命令行运行：`python bse_ipo_valuation.py 920177`

程序会提示输入 6 位代码，生成带时间戳的 PDF 报告到 `输出/` 目录。

## 当前核心文件

- `bse_ipo_valuation.py`：主入口
- `report_generator.py`：报告渲染与 PDF 导出
- `valuation_engine.py`：方法一、方法二、综合估值
- `data_fetcher.py`：IPO 基础信息与近期样本获取
- `pdf_parser.py`：公告文件解析
- `trend_scorer.py`：首日走势评分
- `wind_helper.py`：当前保留的数据源适配层
...
- 目前保留的是“可运行入口 + 核心链路 + 当前验证资产”
- 后续如果切换数据源，优先从 `data_fetcher.py` 和 `wind_helper.py` 接续
