...
- PDF 解析需要 `requirements.txt` 中的依赖。首次安装可运行 `python -m venv .venv`，然后运行 `.venv\Scripts\python.exe -m pip install -r requirements.txt`；`运行.bat` 会优先使用该项目环境。
...
3. 官网查询失败时，自动尝试东方财富新版公告备用源，按证券代码匹配最近 100 条公告中的完整招股文件、发行公告等；若招股说明书仍缺失，则及时终止本次报告
4. 若上市公告书缺失，但招股说明书已拿到，则报告仍可继续生成

遇到官网 HTTP 302 循环重定向时，程序会停止重复请求并尝试备用源。备用源保留公告详情提供的完整附件链接；若公开附件 HTTPS 返回验证页或下载失败，仅对 `pdf.dfcfw.com` 的公开公告 PDF 尝试同域名 HTTP 地址，并校验 PDF 文件头、结束标记和响应长度。两边都失败时，可把完整 PDF 保存到 `公告文件/`，命名为 `6位代码_公司简称_招股说明书.pdf`，再运行 `运行.bat`。
...
近期资金水位采用无前视自适应口径：对最近已公布结果的历史新股，逐只用它之前的样本重建当时预测，以“真实冻资 / 未做水位修正的预测冻资”生成残差因子；同一结果日样本不互相引用。系统按样本半衰期取加权中位数，并把相对原公式因子 `1.0` 的残差按自适应权重收缩后作为有限加成，最终因子受配置上下限保护。`subscription_prediction_recent_market_level_factor` 只在样本不足时回退；自适应启用后自动调参不会继续上调该静态基准，避免两层参数重复吸收同一段近期偏差。样本齐备后，正式预测和调参回放都使用同一套滚动因子。默认 `coarse-fine` 搜索会调节是否启用、近期窗口、最少样本、半衰期和自适应权重，候选排序以资金规模综合误差为先，并同时复核分档 MAPE 和近期有符号偏差。
